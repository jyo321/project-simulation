...
# see the matching listener-rule conditions in modules/ecs.
...
resource "aws_cloudfront_origin_access_control" "spa" {
  ...
  name                              = "northbridge-${each.key}-${var.environment}-oac"
...
resource "aws_cloudfront_distribution" "spa" {
  ...
  origin {
    ...
    custom_origin_config {
      http_port              = 80
      https_port             = 443
      origin_protocol_policy = "http-only" # ALB only listens on 80 — see modules/ecs
  ...
  # forwarded untouched since the ALB's own path-based routing (modules/ecs) needs the
  # real path.

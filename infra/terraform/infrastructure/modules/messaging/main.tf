...
resource "aws_sns_topic" "application_events" {
  name = "northbridge-application-events-${var.environment}"
}

resource "aws_sns_topic" "ops_alerts" {
  name = "northbridge-ops-alerts-${var.environment}"
}
...
resource "aws_sqs_queue" "document_validation_dlq" {
  name = "document-validation-dlq-${var.environment}"
}

resource "aws_sqs_queue" "document_validation" {
  name                       = "document-validation-${var.environment}"
...
resource "aws_sqs_queue" "status_projector_dlq" {
  name = "status-projector-dlq-${var.environment}"
}

resource "aws_sqs_queue" "status_projector" {
  name                       = "status-projector-${var.environment}"
...
resource "aws_sqs_queue" "notifications_dlq" {
  name = "notifications-dlq-${var.environment}"
}
...
resource "aws_cloudwatch_metric_alarm" "notifications_dlq_depth" {
  alarm_name          = "northbridge-notifications-dlq-depth-${var.environment}"
...
resource "aws_sqs_queue" "credit_scoring_jobs_dlq" {
  name = "credit-scoring-jobs-dlq-${var.environment}"
}

resource "aws_sqs_queue" "credit_scoring_jobs" {
  name = "credit-scoring-jobs-${var.environment}"
...
resource "aws_sqs_queue" "fraud_analysis_jobs_dlq" {
  name = "fraud-analysis-jobs-dlq-${var.environment}"
}

resource "aws_sqs_queue" "fraud_analysis_jobs" {
  name                       = "fraud-analysis-jobs-${var.environment}"
  visibility_timeout_seconds = 120
...
resource "aws_scheduler_schedule" "daily_stale_report" {
  name                         = "northbridge-daily-stale-report-${var.environment}"
  ...
  target {
    arn      = var.daily_stale_report_lambda_arn
    role_arn = var.scheduler_run_task_role_arn
...
resource "aws_pipes_pipe" "credit_scoring" {
  name     = "northbridge-credit-scoring-pipe-${var.environment}"
  role_arn = var.pipes_run_task_role_arn
  source   = aws_sqs_queue.credit_scoring_jobs.arn
  target   = var.ecs_cluster_arn

  target_parameters {
    ecs_task_parameters {
      task_definition_arn = var.credit_scoring_job_task_definition_arn
      launch_type         = "FARGATE"
...
resource "aws_pipes_pipe" "fraud_forensics" {
  name     = "northbridge-fraud-forensics-pipe-${var.environment}"
  role_arn = var.pipes_run_task_role_arn
  ...
  target_parameters {
    ecs_task_parameters {
      task_definition_arn = var.fraud_forensics_job_task_definition_arn
      launch_type         = "FARGATE"

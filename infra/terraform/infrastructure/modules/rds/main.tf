resource "aws_db_subnet_group" "main" {
  name       = "northbridge-db-subnets-${var.environment}"
...
resource "aws_db_instance" "postgres" {
  identifier     = "northbridge-postgres-${var.environment}"
  engine         = "postgres"
  engine_version = "16.4"
  ...
  final_snapshot_identifier = "northbridge-postgres-final-${var.environment}-${random_id.rds_snapshot_suffix.hex}"

  tags = { Name = "northbridge-postgres-${var.environment}" }
}
